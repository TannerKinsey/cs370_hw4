...
int main(int argc, char **argv)
{
    ...
    for(int i = 0; i <= x; i++){
        ...
        if(pid > 0){//parent process
            ...
            //Step 1
            //this array is set up to determine the read end or the write end for the pipe
            int readwrite[2];
            pipe(readwrite);
            //read-write[0] -> for using read end
            //read-write[1] -> for using write end
            
            //printf("IDK: %d\n", pipe(readwrite));
            
            //Step 2
            //create char array of 100 that will store the string format of the pipe location 
            char stringPipeLocation[100];
            sprintf(stringPipeLocation, "%d", readwrite[1]);
            printf("%s\n", stringPipeLocation);
